<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Welcome To Javascript To React Js Code</title>
    <link rel="icon" href="mission.png" type="image/x-icon">
    <script src="1-welcome.js"></script>
    <script src="2-datatype.js"></script>
    <script src="3-let-var-const.js"></script>
    <script src="4-object.js"></script>
    <script src="5-operators.js"></script>
    <script src="6-alert-prompt.js"></script>
    <script src="7-condition-statements.js"></script>
    <script src="8-for-loop-while-do-while.js"></script>
    <script src="9-string.js"></script>
    <script src="10-template-literals.js"></script>
    <script src="11-array.js"></script>
    <script src="12-function-arror-function.js"></script>
    <script src="13-for-each-loop.js"></script>
    <script src="14-map.js"></script>
    <script src="15-filter.js"></script>
    <script src="16-reduce.js"></script>
    <script src="17-dom.js"></script>
    <script src="18-event.js"></script>
    <script src=""></script>
    <script src=""></script>
    <script src=""></script>
    <script src=""></script>
</head>

<body>
    <h1>Wellcome Suraj</h1>
    <p>This is Javascript</p>


    <!-- Dom -->
    <p id="text">This is DOM : Original Text</p>
    <button onclick="changeText()">Click Me</button>

    <br><br>

    <!-- EventListener -->
    <p>This is EventListner</p>
    <button onclick="sayHello()">Click Here</button>

    <br><br>

    <!-- EventListener -->
    <form id="myForm">
        <p>This is form</p>
        <input type="text" id="name" />
        <button type="submit">Submit</button>
    </form>


</body>

</html>